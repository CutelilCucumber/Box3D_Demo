...
func run(ctx: Variant) -> void:
	...
	ctx.log("uses_cannonball: " + str(mech.uses_cannonball()))
	...
	ctx.log("uses_cannonball after [: " + str(mech.uses_cannonball()))
	...
	# Test 4: Fire while moving (check if cannonball vs plasma)
	ctx.log("=== Test 4: Fire while moving ===")
	ctx.press_key(KEY_W)
	await ctx.wait(0.2)
