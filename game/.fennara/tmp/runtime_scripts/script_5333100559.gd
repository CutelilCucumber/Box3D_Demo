...
func run(ctx: Variant) -> void:
	...
	var _bolt = LightPlasma.spawn(world, gun_tip, dir * 32.0, 10.0)
	...
	for n in world.get_children():
		if n is Box3DBody:
			for c in n.get_children():
				if c is Decal:
					decal_count += 1
					ctx.log("decal on body %s at %s size=%s modulate=%.2f" % [n.name, str(c.global_position), str(c.size), c.modulate.a])
